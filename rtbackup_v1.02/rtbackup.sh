...
# creates auto_backup dir
mkdir $backup_location/auto_backup &> /dev/null
...
# prints the realtime status of max_backup, user, backup_location, and the --help
  ...
  "--status")
          ...
          echo -e ">>>     backup location [\e[32m$(pwd)/$backup_location\e[0m]"
                ...
                printf ">>>\tfree space in current directory [\e[32m$disk_free\e[0m] \t(G=GigaBytes|K=KiloBytes|M=MegaBytes)\n"

                # shows the current logged in user
          echo -e ">>>     logged in as user [\e[32m$id_cmd\e[0m]"
...
# prints the help message for further use case
  "--help")
          ...
          echo ">>>     --restore-auto (restores recent auto_backup from backup location)"
          echo ">>>     --restore (restores recent backup from manual backups)"
          echo -e ">>>     --uninstall (\e[31mremoves every files\e[0m related to rtbackup tool)"
          printf "\n"
          ;;
# finds all of tar.gz files in default backup location and deletes oldest tar.gz after creating log
  ...
  "--uninstall")
		...
		read -p "Do you want to $( echo -e "\e[31mdelete all the files\e[0m") related to rtbackup? [y/n]: " response_rt

                # deletes all the files related to rtbackup tool
                if [ $response_rt == 'y' ]; then
			$(rm -r backup.conf backups log .auto_conf .location_conf &> /dev/null)
			echo "cleaning done."
  ...
  "--auto-backup")
          ...
          if [ $auto_backup_rep == 'y' ]; then
                  ...
                  chmod 0770 "$(pwd)/$backup_location/auto_backup" &> /dev/null
  ...
  "--restore-auto")
	    # this will pick recent auto_backup file from backup location created by backup.conf, if doesn't exist then throw an error.
            local_tar_dir_auto="$(pwd)/$backup_location/auto_backup"
            auto_recent_auto=$(cd $local_tar_dir_auto && ls | grep tar | tail -1)
            check_auto_recent_auto=$(cd $backup_location_auto && ls | grep tar &> /dev/null; echo $?)
            if [ $check_auto_recent_auto -eq 0 ]; then
                    echo -e "Most recent backup >>>       \e[32m$auto_recent_auto\e[0m"
                    read -p "Proceed with auto_restore? [y/n]: " auto_recent_rep_auto
                    if [ "$auto_recent_rep_auto" == 'y' ]; then
                        cd $local_tar_dir_auto && tar -xvf $auto_recent_auto -C / && echo -e "\e[32mauto restore completed. \ncheck log/restore.log file for more details.\e[0m"
                        echo -e "date:[$date]\nrestoredfrom:[$auto_recent_auto] \nuser:[$id_cmd]\n\n" >> $conf_dir/log/restore.log
                    else
                        echo "cancelling auto restore..."
                    fi
            else
                     echo -e "\e[31mNo backups found\e[0m"
            fi
  ...
  "--restore")
            ...
            auto_recent_usr=$(cd $local_tar_dir_usr && ls | grep tar | tail -1)
            check_auto_recent_usr=$(cd $backup_location && ls | grep tar &> /dev/null; echo $?)
            if [ $check_auto_recent_usr -eq 0 ]; then
                    echo -e "Most recent backup >>>       \e[32m$auto_recent_usr\e[0m"
                    read -p "Proceed with auto_restore? [y/n]: " auto_recent_rep_usr
                    if [ "$auto_recent_rep_usr" == 'y' ]; then
                        cd $local_tar_dir_usr && tar -xvf $auto_recent_usr -C / && echo -e "\e[32mrestore completed. \ncheck log/restore.log file for more details.\e[0m"
